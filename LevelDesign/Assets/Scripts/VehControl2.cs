...
public class VehControl2 : MonoBehaviour
{
	...
	void Accelerate()
	{
		...
		{
			RL.motorTorque = torque * Input.GetAxis("Vertical2");
			RR.motorTorque = torque * Input.GetAxis("Vertical2");
			RL.brakeTorque = 0;
			RR.brakeTorque = 0;
		...
		if (engineRPM > 0 && Input.GetAxis("Vertical2") < 0 && engineRPM <= gearUpRPM)
		{
	...
	void Steer()
	{
		...
		FL.steerAngle = SteerAngle * Input.GetAxis("Horizontal2");
		FR.steerAngle = SteerAngle * Input.GetAxis("Horizontal2");
	}
